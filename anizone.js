...
async function extractStreamUrl(url) {
    try {
        ...
        try {
            ...
            // Make all relative URIs absolute and set English as default audio
            const absoluteM3u8 = m3u8Text
                .replace(
                    /(URI="|URI=')([^"']+)("|')/g,
                ...
                // Set English as default, all others non-default
                .replace(
                    /(#EXT-X-MEDIA:[^\n]*LANGUAGE="en"[^\n]*)(DEFAULT=YES)/g,
                    '$1DEFAULT=YES'
                )
                .replace(
                    /(#EXT-X-MEDIA:[^\n]*)DEFAULT=(YES|NO)/g,
                    (match, prefix) => {
                        if (/LANGUAGE="en"/.test(prefix)) {
                            return prefix + 'DEFAULT=YES';
                        }
                        return prefix + 'DEFAULT=NO';
                    }
                );
