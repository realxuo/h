...
async function extractStreamUrl(href) {
    try {
        ...
        // 1. Yt-mp4 — decrypts directly to a URL, no clock needed
        ...
        if (ytSrc) {
            try {
                ...
                if (decrypted.startsWith("http")) {
                    streamUrl = decrypted;
                    console.log("[allanime] Yt-mp4 direct: " + streamUrl);
                }
            } catch(e) { console.log("[allanime] Yt-mp4 failed: " + e); }
        }

        // 2. Clock-based sources (S-mp4, Luf-Mp4, Default)
        if (!streamUrl) {
            var clockSrc = sourceUrls.find(function(s) {
                return s.sourceName === "S-mp4" || s.sourceName === "Luf-Mp4" || s.sourceName === "Default";
            });
            if (clockSrc) {
                try {
                    ...
                    var links = clockData.links || [];
                    var preferred = links.find(function(l) { return l.mp4 || (l.link && l.link.includes("sharepoint")); });
                    if (!preferred) preferred = links.find(function(l) { return l.link && !l.link.includes("fast4speed"); });
                    if (!preferred && links.length) preferred = links[0];
                    if (preferred) streamUrl = preferred.link || preferred.src || "";
