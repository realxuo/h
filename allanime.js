...
async function extractStreamUrl(href) {
    try {
        ...
        const endpoint = "https://cdn.allanime.day/blog";
        ...
        // Prefer SharePoint or non-fast4speed links, fallback to first
        var preferred = links.find(function(l) { return l.link && l.link.includes("sharepoint.com"); });
        if (!preferred) preferred = links.find(function(l) { return l.link && !l.link.includes("fast4speed"); });
        if (!preferred) preferred = links[0];
        const streamUrl = preferred.link || "";
        ...
        console.log("[allanime] stream URL: " + streamUrl);
        return JSON.stringify({
            streams: [{ title: "AllAnime (DUB)", streamUrl: streamUrl }],
            subtitle: ""
        });
    } catch (e) {
