...
async function extractEpisodes(url) {
    // url = series content_id e.g. "3MBUgbIO"
    try {
        console.log("[extractEpisodes] fetching series: " + url);
        const res  = await soraFetch(`${API}/series/${url}`);
        const data = await res.json();

        const seasons = data.seasons || [];
        console.log("[extractEpisodes] seasons found: " + seasons.length);
        ...
        const seasonResults = await Promise.all(seasons.map(async (season) => {
            ...
            const pages     = Math.ceil(epCount / 50);
            const eps       = [];
            ...
            for (let page = 1; page <= pages; page++) {
                try {
                    ...
                    for (const ep of (epList || [])) {
                        ...
                        eps.push({
                            title:  ep.title ? `S${seasonNum}E${epNum} - ${ep.title}` : `Season ${seasonNum} Episode ${epNum}`,
                            href:   `${seasonId}|${ep.content_id}`,
                            number: parseFloat(epNum)
                        });
                    }
                } catch (e) {
                    console.log(`[extractEpisodes] season ${seasonNum} page ${page} error: ` + e);
                }
            }
            return eps;
        }));

        const episodes = seasonResults.flat();
        console.log("[extractEpisodes] total episodes: " + episodes.length);
